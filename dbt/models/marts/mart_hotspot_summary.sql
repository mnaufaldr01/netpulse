...
as_of as (
    select max(event_date) as as_of_date from telemetry
),
events as (
...
agg as (
    select
        t.tower_id,
        count(distinct case
            when e.severity is not null
             and e.event_hour::date >= a.as_of_date - interval '7 days'
            then e.event_hour end)::float
            / nullif(count(distinct case
                when t.event_hour::date >= a.as_of_date - interval '7 days'
                then t.event_hour end), 0) * 100
            as congestion_frequency_7d,
        count(distinct case
            when e.severity is not null
             and e.event_hour::date >= a.as_of_date - interval '30 days'
            then e.event_hour end)::float
            / nullif(count(distinct case
                when t.event_hour::date >= a.as_of_date - interval '30 days'
                then t.event_hour end), 0) * 100
